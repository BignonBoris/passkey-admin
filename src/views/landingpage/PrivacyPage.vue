...
import { PhFilePdf, PhDownloadSimple } from '@phosphor-icons/vue'
...
const { tm } = useI18n()
const sections = computed(() => tm('privacy.sections') as ContentSection[])
      ...
      <div class="max-w-4xl mx-auto px-6">
        <div class="text-center mb-12">
          <div class="inline-flex items-center justify-center w-20 h-20 rounded-[28px] bg-blue-50 text-blue-900 mb-6 shadow-sm border border-blue-100">
            <PhFilePdf :size="40" weight="duotone" />
          </div>
          <h1 class="text-4xl md:text-5xl font-black mb-6 text-slate-900">{{ $t('privacy.title') }}</h1>
          <p class="text-slate-500 text-lg leading-relaxed">{{ $t('privacy.subtitle') }}</p>
        </div>
        ...
        <div class="bg-white p-8 md:p-12 rounded-[40px] shadow-sm border border-slate-100 mb-10">
          ...
          <div class="pt-12 border-t border-slate-100 flex flex-col items-center gap-6">
            <div class="text-center">
              <h3 class="text-2xl font-black mb-2 text-slate-900">{{ $t('privacy.card_title') }}</h3>
              <p class="text-slate-500 mb-8 leading-relaxed max-w-md mx-auto">{{ $t('privacy.card_desc') }}</p>
            </div>
