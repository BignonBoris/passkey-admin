...
<template>
  <div class="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-orange-100">
    ...
    <main class="pt-32 pb-20">
      <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="text-center mb-16">
          <h1 class="text-4xl md:text-6xl font-black mb-6 text-slate-900">{{ $t('eats.contact.title') }}</h1>
          <p class="text-slate-500 text-lg max-w-2xl mx-auto leading-relaxed">{{ $t('eats.contact.subtitle') }}</p>
        </div>
        ...
        <div class="grid lg:grid-cols-3 gap-12">
          ...
          <div class="lg:col-span-1 space-y-8">
            <div class="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
              <h2 class="text-2xl font-black mb-8 text-slate-900">{{ $t('eats.contact.info_title') }}</h2>
              
              <div class="space-y-6">
                <!-- Address -->
                <div class="flex gap-4">
                  <div class="w-12 h-12 rounded-2xl bg-[#011c61]/10 flex items-center justify-center text-[#011c61] shrink-0">
                    <PhMapPin :size="24" weight="duotone" />
                  </div>
                  <div>
                    <h3 class="font-black text-slate-900 mb-1">{{ $t('eats.contact.info_address') }}</h3>
                ...
                <div class="flex gap-4">
                  ...
                  <div>
                    <h3 class="font-black text-slate-900 mb-1">{{ $t('eats.contact.info_phone') }}</h3>
                ...
                <div class="flex gap-4">
                  ...
                  <div>
                    <h3 class="font-black text-slate-900 mb-1">{{ $t('eats.contact.info_email') }}</h3>
              ...
              <div class="mt-12 pt-8 border-t border-slate-100">
                <div class="flex gap-4">
                  <a href="https://web.facebook.com/akasiholding" target="_blank" class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-blue-600 hover:text-white transition-all shadow-sm">
            ...
            <div class="bg-white p-8 md:p-12 rounded-[32px] shadow-sm border border-slate-100">
              <form @submit.prevent="handleSubmit" class="space-y-6">
                <div class="grid md:grid-cols-2 gap-6">
                  <!-- Name -->
                  <div>
                    <label class="block text-sm font-black text-slate-900 mb-2">{{ $t('eats.contact.form_name') }}</label>
                    <input v-model="form.name" type="text" required class="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#011c61] focus:border-transparent transition-all" />
                  </div>
                  <!-- Email -->
                  <div>
                    <label class="block text-sm font-black text-slate-900 mb-2">{{ $t('eats.contact.form_email') }}</label>
                    <input v-model="form.email" type="email" required class="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#011c61] focus:border-transparent transition-all" />
                  </div>
                </div>

                <!-- Subject -->
                <div>
                  <label class="block text-sm font-black text-slate-900 mb-2">{{ $t('eats.contact.form_subject') }}</label>
                  <input v-model="form.subject" type="text" required class="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#011c61] focus:border-transparent transition-all" />
                </div>

                <!-- Message -->
                <div>
                  <label class="block text-sm font-black text-slate-900 mb-2">{{ $t('eats.contact.form_message') }}</label>
                  <textarea v-model="form.message" rows="5" required class="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#011c61] focus:border-transparent transition-all resize-none"></textarea>
                </div>

                <!-- Submit -->
                <div>
                  <button type="submit" :disabled="isSubmitting" 
                    class="w-full md:w-auto bg-[#011c61] text-white px-10 py-4 rounded-2xl font-black text-sm shadow-xl shadow-[#011c61]/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                    <span v-if="!isSubmitting">{{ $t('eats.contact.form_submit') }}</span>
              ...
              <SwiperSlide v-for="office in offices" :key="office.name">
                <div class="bg-white rounded-[32px] overflow-hidden shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col h-full hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                  <div class="h-64 relative overflow-hidden">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-orange-950/40 to-transparent"></div>
                  </div>
                  
                  <div class="p-8 flex flex-col items-center text-center flex-1">
                    <h3 class="text-xl font-black text-[#011c61] mb-6 uppercase">{{ office.name }}</h3>
